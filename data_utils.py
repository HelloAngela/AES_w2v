...
# TODO: change path of glove word embeddings
def load_glove(token_num=6, dim=300):
    word2vec = []
    word_idx = {}                       #dictionary {} mapping word to a index in the essay
    # first word is nil
    ...
    #                                                                    "B." + str(dim) + "d.txt")) as f:
        ...
        for line in f:
            l = line.split()
            word = l[0]                                 #index of the word is 0, first item is the word
            vector = list(map(float, l[1:]))            #map(function to be applied, on object); 300 vectors
            word_idx[word] = count                      #count=1, so word count  is 1,
