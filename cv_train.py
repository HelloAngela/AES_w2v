...
# flags, might need to add tf.compat.v1.flags
tf.compat.v1.flags.DEFINE_float("epsilon", 0.1, "Epsilon value for Adam Optimizer.")
tf.compat.v1.flags.DEFINE_float("l2_lambda", 0.3, "Lambda for l2 loss.")
tf.compat.v1.flags.DEFINE_float("learning_rate", 0.001, "Learning rate")
tf.compat.v1.flags.DEFINE_float("max_grad_norm", 10.0, "Clip gradients to this norm.")
tf.compat.v1.flags.DEFINE_float("keep_prob", 0.9, "Keep probability for dropout")
...
tf.compat.v1.flags.DEFINE_integer("essay_set_id", 1, "essay set id, 1 <= id <= 8")
tf.compat.v1.flags.DEFINE_integer("token_num", 42, "The number of token in glove (6, 42)")
tf.compat.v1.flags.DEFINE_boolean("gated_addressing", False, "Simple gated addressing")
tf.compat.v1.flags.DEFINE_boolean("allow_soft_placement", True, "Allow device soft device placement")
tf.compat.v1.flags.DEFINE_boolean("is_regression", False, "The output is regression or classification")
tf.compat.v1.flags.DEFINE_boolean("log_device_placement", False, "Log placement of ops on devices")
# hyper-parameters


# The model is trained on the training data with 5-fold cross validation.
# By default, the output layer of the model is a classification layer.
# There is another model whose output layer is a regression layer in memn2n_kv_regression.py.
# To train the model with the regression output layer, set flag is_regression to True. For example,
...
f = open(out_dir+file_name+'_out.txt', 'w')
sys.stdout = f
print("Writing to {}\n".format(out_dir))
...
training_path = "C:/Users/hello/Desktop/2018 paper/txt_as_list.csv"
...
# I guess the above is to build our own vocab rather than using word2vec? NG
